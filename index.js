...
const processAccount = async (authorization, autoClearTasks, autoMaxLevelUp) => {
    try {
        console.log(getTimeStamp() + 'Processing account...');
        await delay(1000);

        if (autoMaxLevelUp) {
            const levelUpStatus = await levelUp(authorization);
        ...
        while (true) {
            ...
            const tapResult = await tap(authorization, clicks);
            await delay(1000);

            const barAmount = await getBarAmount(authorization);
            await delay(1000);

            const boxMall = await getBoxMall(authorization);
            await delay(1000);
        ...
        console.error(getTimeStamp() + 'Error processing account', error);
    }
};
...
const main = async () => {
    try {
        ...
        const validToken = 'VALID_TOKEN_HERE'; // Replace this with your valid token
        while (true) {
            for (const accountId of accountIds) {
                await processAccount(validToken, autoClearTasks, autoMaxLevelUp);
            }
